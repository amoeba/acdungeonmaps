...
const TILES_QUERY = function (id: string): string {
  return `select+x%2C+y%2C+z%2C+environment_id+%2Cname+from+${TILES_TBL_NAME}+left+join+${DUNGEONS_TBL_NAME}+on+${DUNGEONS_TBL_NAME}.landblock_id+%3D%3D+${TILES_TBL_NAME}.landblock_id+where+${TILES_TBL_NAME}.landblock_id+%3D+%27${id}%27+%3B`;
}
